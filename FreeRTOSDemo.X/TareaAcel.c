...
void LeerAcel(void *pvParameters)
{
    ...
    while (1)
    {
        ...
        if ((l_x * l_x + l_y * l_y) < 100) // si el módulo es muy grande es que hay aceleraciones externas
        {
            ...
            if (fabs(angulo_ant_ant - angulo_ant) >= TOLERANCIA && fabs(angulo_act - angulo_ant) <= TOLERANCIA)
            {
